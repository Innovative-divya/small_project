...
let shon = document.getElementById("shon");
let anidur = 15;
let dealy = 2;
let w = 6;
let h = 9;
...
function hh() {
    ...
    let left = 1290;
    let z = 629;
    let shons = document.getElementById("container");

    for (let i = 0; i < 250; i++) {
        let span = document.createElement("span");
        span.setAttribute("class", "shons");
        let time = Math.floor(Math.random() * anidur);
        let d = Math.floor(Math.random() * dealy);
        let ww = Math.floor(Math.random() * w);
        let hh = Math.floor(Math.random() * h);
        let l = Math.floor(Math.random() * left);
        let zz = Math.floor(Math.random() * z);
        span.style.animationDuration = time + 1 + "s";
        span.style.animationDelay = d + "s";
        span.style.width = ww + "px";
        span.style.height = hh + "px";
        span.style.transform = `translateZ(${zz}px)`;
        if (l <= 173) {
            span.style.left = l + 50 + "px";
        } else {
            span.style.left = l + "px";
        }
        shons.appendChild(span);
        // console.log(span)
    }
}
hh()